...
def get_var_names(
    data, group: str = "posterior", dims: Union[str, Tuple[str]] = "all"
) -> List[str]:
    ...
    if var_names is None:
        ...
        if len(var_names) == 0:
            if dims == "all":
                ...
                msg = (
                    f"No variables exist for dims {dims}"
                    + f" in group '{group}'."
                )
        ...
        if not available.intersection(subset) == subset:
            ...
            raise ValueError(
                f"Variable name(s) {diff} not available"
...
def get_summary(
    ...
    **kwargs,
    ...
    stat_funcs = {
        ...
        "16th": lambda x: np.quantile(x, 0.16),
        "50th": np.median,
        "84th": lambda x: np.quantile(x, 0.84),
    }
    ...
    circ_var_names = kwargs.pop("circ_var_names", circ_var_names)
    summary = az.summary(
        data,
        group=group,
        var_names=var_names,
...
def get_table(
    ...
    **kwargs,
    ...
    var_names = _validate_var_names(
        data, group=group, var_names=var_names, dims=dims
    )
    ...
    if round_to == "auto":
        ...
        if "sd" not in summary.metric:
            raise ValueError(
                "Automatic rounding requires the standard " + "deviation 'sd'."
            )
        ...
        if isinstance(table.index, pd.MultiIndex):
            precision = (
                precision.min()
            )  # Choose min precision = max decimal precision
